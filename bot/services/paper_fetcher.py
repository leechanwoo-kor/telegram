...
async def fetch_paper_abstract(paper_url):
    logger.info(f"Fetching abstract for: {paper_url}")
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(paper_url) as response:
                if response.status == 200:
                    ...
                    if abstract_header:
                        ...
                        if abstract_text:
                            logger.info(f"Successfully fetched abstract for {paper_url} (new method).")
                            return abstract_text.strip()
                    
                    abstract_tag = soup.find("p", class_="text-gray-700 dark:text-gray-400")
                    if abstract_tag: # 기존 방식의 초록 태그 검색 (class 사용)
                        logger.info(f"Successfully fetched abstract for {paper_url} (old method).")
                    ...
                    logger.warning(f"Failed to fetch abstract page {paper_url}. Status: {response.status}")
    except Exception as e:
        logger.error(f"Error fetching paper abstract for {paper_url}: {e}")
    return "초록을 찾을 수 없음"
